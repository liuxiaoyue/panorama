...
var DeviceOrientationController = function ( object, domElement ) {
	...
	this.isIos =  userAgent.match(/iphone|ipod/i);
	...
	this.updateDeviceMove = function () {
		...
		var tempObj = {};
		return function () {
			tempObj.alpha = this.deviceOrientation.alpha;
			tempObj.beta = this.deviceOrientation.beta;
			tempObj.gamma = this.deviceOrientation.gamma;

			alpha  = THREE.Math.degToRad( tempObj.alpha || 0 ); // Z
			beta   = THREE.Math.degToRad( tempObj.beta  || 0 ); // X'
			gamma  = THREE.Math.degToRad( tempObj.gamma || 0 ); // Y''
			...
			if ( alpha !== 0 && beta !== 0 && gamma !== 0) {
				...
				//此处针对 ios android有调整，android下直接copy页面存在抖动效果
				if(this.isIos){
